...
const AddChannel = ({ channelGroupId }:{channelGroupId: string | null}) => {
  const currentServer = useServerBoundStore((state) => state.currentServer);
  const { mutateAsync, data } = useAddChannel(currentServer?.id ?? null);
  ...
  return <Form form={form} onSubmit={(data) => mutateAsync({...data, channelGroupId })} className="flex flex-col gap-y-3">
      <TextInput
        name="name"
        label="채널 이름"
